...
class TraitGenerator:
  ...
  def generate_items( self, new_items ):
    ...
    gen_items = []
    all_items = [*self.continue_items, *new_items]
  ...
  def randomize_last( self, new_items ):
    pass
  ...
  @classmethod
  def is_allowed( cls, item, rule ):
    for allow in rule['allowed']:
      if cls.is_match( item, allow ):
        try:
          rule['hits'] += 1
        except KeyError:
          rule['hits'] = 1
  ...
  @classmethod
  def is_denied( cls, item, rule ):
    for denied in rule['denied']:
      if cls.is_match( item, denied ):
        try:
          rule['hits'] += 1
        except KeyError:
          rule['hits'] = 1
  ...
  def is_match( item, match ):
    if TraitGenerator.is_enabled( match ):
  ...
  def is_valid( self, item, throw=False ):
    ...
    for rule in self.rules:
      ...
      if not self.is_match( item, rule['match'] ):
        continue

      if rule['type'] == 'allow':
        if not self.is_allowed( item, rule ):
          if throw:
...
if __name__ == '__main__':
  ...
  try:
    ...
    #gen.check_distribution( new_items )
